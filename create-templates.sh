...
import_disk() {
    ...
    local image_file=$2
    # 导入磁盘到 local 存储
    echo "正在导入磁盘 /tmp/$image_file 到 local 存储..."
    qm importdisk $vmid /tmp/$image_file local --format qcow2
    if [ $? -ne 0 ]; then
    ...
    if [ ! -f "/var/lib/vz/images/$vmid/vm-$vmid-disk-0.qcow2" ]; then
        echo "错误：导入的磁盘文件 /var/lib/vz/images/$vmid/vm-$vmid-disk-0.qcow2 不存在"
        exit 1
    fi
...
configure_vm() {
    local vmid=$1
    # 设置 SCSI 控制器并挂载导入的磁盘
    echo "正在挂载磁盘 local:$vmid/vm-$vmid-disk-0.qcow2 到 scsi0..."
